...
import purchaseZBDetailsSearch from '../../components/Inventory/PurchaseZB/searchAdd';
import purchaseZBDetailsList from '../../components/Inventory/PurchaseZB/listAdd';
import purchaseZBDetailsModal from '../../components/Inventory/PurchaseZB/modal';

const purchaseZBDetails = ({ purchaseZBData, dispatch }) => {
  const {
    loading,
    storeId, // 机构id
...
// console.log("aclStoreListAll",aclStoreListAll);
  const purchaseZBDetailsListProps = { // 新增列表
    loading,
    storeId,
    pagination, // 首页分页
    dataList,
    ...
    onPageChange(page) { // 修改分页
      dispatch({
        type: 'purchaseZB/findScmInGoods',
        payload: {
          page: page.current,
          rows: page.pageSize,
          limit: '20',
    ...
    queryGoodsCoding(queryString) { // 查找编码
      dispatch({
        type: 'purchaseZB/queryGoodsCoding',
    ...
    onSave(value) {
      ...
      if (update) { // 编辑
        dispatch({
          type: 'purchaseZB/addScmDirectManual',
          payload: {
            id: dataAll.id,
            bussDate, // 订单日期
  ...
  const purchaseZBDetailsSearchProps = { // 新增搜索
  ...
  const purchaseZBDetailsModalProps = {
    ...
    okHideModal() {
      ...
      dispatch({
        type: 'purchaseZB/exportModalListAll',
        payload: {},
      });
    },
  };
  return (
    <div className="routes">
      <purchaseZBDetailsSearch {...purchaseZBDetailsSearchProps} />
      <purchaseZBDetailsList {...purchaseZBDetailsListProps} />
      <purchaseZBDetailsModal {...purchaseZBDetailsModalProps} />
    </div>
  );
};
purchaseZBDetails.propTypes = {
...
export default connect(mapStateToProps)(purchaseZBDetails);
