...
export async function querySupplyList(params) {
  return request('/ipos-chains/scmzb/scmDirect/findDataForPage', {
    method: 'post',
    body: JSON.stringify(params),
  });
}
...
export async function queryExport(params) {
  ...
  const url = '/ipos-chains/scmzb/export/exportScmDirect?conditionJsonStr=%7B'.concat(paramFormatJson(params), '%7D');
  window.open(url);
}


export async function update(params) {
  return request('/ipos-chains/scmzb/scmDirect/toUpdate', {
    method: 'post',
    body: JSON.stringify(params),
  });
}
