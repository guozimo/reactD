...
class MemberDetails extends React.Component {
  ...
  handleSearch = (e) => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      this.props.dispatch({
        type: 'rechargeDetails/fetchRechargeReport',
        values,
      });
    });
  }
  ...
  exportExcel = (excelValue) => {
    ...
    if (excelValue) {
      this.props.dispatch({
        type: 'rechargeDetails/exportExcel',
        excelValue,
      });
    } else {
      console.log(2);
  ...
  render() {
    ...
    return (
      <div>
        ...
        <RechargeTable {...tableRrcharge} />
      </div>
    );
  }
}
function mapStateToProps(state) {
  const { rechargeReportList, typeList, aclStoreList } = state.rechargeDetails;

  return {
    loading: state.loading.models.rechargeDetails,
    rechargeReportList,
    typeList,
    aclStoreList,
  };
