...
const modal = ({
  ...
  // console.warn("supplyId",supplyId);
  const storeListAll = supplierAloneList && supplierAloneList.map(store => <Option value={store.id} key={store.id}>{store.suppName}</Option>);
  return (
    <div>
      <Modal {...modalOpts}>
        <div className="components-modal">
          <Row gutter={16}>
            <Col span={6} style={{ marginBottom: 20 }}>
              <Select style={{ width: 160 }} placeholder="请选择供应商" value={supplyId} onSelect={selectSupplier} onChange={upadateSupplier}>
                <Option value="" key="">请选择供应商</Option>
                {storeListAll}
              </Select>
            </Col>
            {supplyId && <Col span={18} style={{ marginBottom: 20 }}>
              <Button type="primary" icon="plus" onClick={onNewGoodsList}>添加物资编码</Button>
            </Col>}
            {supplyId && <Col span={12}>
              <Table
                size="small"
                className="table"
      ...
      <Modal {...modalVerify}>
        <div>
          您勾选的所有物资将删除原配送机构，修改为由<span style={{ color: 'red' }}>{supplyName}</span>供应商供货的供货关系
          <Table
            size="small"
            className="table"
