...
import { querySupplyList, update, findAclStoreForPage, closeScmDirect, updateScmDirectManual, queryExport, addScmDirectManual } from '../../services/inventory/purchaseZB';

export default {
  namespace: 'purchaseZB',
  state: {
  ...
  effects: {
    ...
    * querySupplier({ payload }, { call, put, select }) {
      ...
      const busiIdAll = yield select(state => state.purchaseZB.newValueShop);
    ...
    * querySupplyList({ payload }, { select, call, put }) {
      ...
      if (data.data && data.data.success) {
        ...
        const distribId = yield select(state => state.purchaseZB.distribId);
        if (distribId) {
          message.warning(`操作失败，请参考：${data.data.errorInfo}`);
          yield put({
            type: 'querySuccess',
    ...
    * queryOrgList({ payload }, { call, put, select }) { // 请求仓库
      // console.warn("我试试进来了吗")
      yield put({ type: 'showLoading' });
      const newValueStoreList = yield select(state => state.purchaseZB.newValueStore);
    ...
    * querySupplyListUpdate({ payload }, { put, select }) {
      // console.warn("我试试进来了吗")
      // const storeId = yield select(state => state.purchaseZB.storeId);
      const { distribId, billListNo, pagination, busiListId, storeListId, listStatus, filterDataRange } = yield select(state => state.purchaseZB);
      const pageNo = payload.pageNo || pagination.current;
      const pageSize = payload.pageSize || pagination.pageSize;
      // const bussDate = yield select(state => state.purchaseZB.bussDate);
      yield put({
    ...
    * update({ payload }, { call, put }) {
      ...
      // const path = '/stock/purchaseZB/supplyOrderAddNew';
    ...
    * queryfind({ payload }, { put }) {
      ...
      const path = `/stock/purchaseZB/supplyOrderFind/${timeId}`;
      yield put(routerRedux.push(path));
    ...
    * addScmDirectManual({ payload }, { call, put, select }) {
      yield put({ type: 'showLoading' });
      const dataSourceNew = yield select(state => state.purchaseZB.dataSourceNew);
      const newUpdate = yield select(state => state.purchaseZB.update);
      // yield put({
    ...
    * addNewList({ payload }, { put }) { // 新增采购订单跳转
      ...
      const timeId = moment().valueOf();
      const path = `/stock/purchaseZB/supplyOrderAddNew/${timeId}`;
    ...
    * editableMem({ payload }, { put, select }) { // 初始化数据
      const data = yield select(state => state.purchaseZB.dataSource);
    ...
    * updateEditableMem({ payload }, { put, select }) { // 回车跳转新的边框
      const stateMem = yield select(state => state.purchaseZB.editableMem);
      const dataSource = yield select(state => state.purchaseZB.dataSource);
    ...
    * addTable({ payload }, { put, select }) { // 新增一行
      let newData = yield select(state => state.purchaseZB.newData);
      const dataSource = yield select(state => state.purchaseZB.dataSource);
    ...
    * findTreeList({ payload }, { call, put }) { // 查找物资类别
      // const depotId = yield select(state => state.purchaseZB.depotId);
      // const storeId = yield select(state => state.purchaseZB.storeId);
      yield put({ type: 'showLoading' });
    ...
    * exportModalListAll({ payload }, { put, select }) { // 选择全部数据
      const newArrayAll = yield select(state => state.purchaseZB.dataSource);
      const newValue = yield select(state => state.purchaseZB.exportModalList);
      const modalRowIndex = yield select(state => state.purchaseZB.modalRowIndex);
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        ...
        if (location.pathname === '/stock/purchaseZB') {
        ...
        const reFind = pathToRegexp('/stock/purchaseZB/supplyOrderFind/:storeIdFindKey');
        ...
        const re = pathToRegexp('/stock/purchaseZB/supplyOrderAddNew/:storeIdKey');
        ...
        // if (location.pathname === '/stock/purchaseZB/supplyOrderAddNew') {
