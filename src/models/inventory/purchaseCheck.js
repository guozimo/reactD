...
import { querySupplyList, update, findAclStoreForPage, closeScmDirect, updateScmDirectManual, queryExport, addScmDirectManual } from '../../services/inventory/purchaseCheck';

export default {
  namespace: 'purchaseCheck',
  state: {
    ...
    storeListId: '',
    distribId: '', // 机构id
    filterDataRange: [moment(), moment()], // 采购日期
    loading: false,
    typeList: [{ // 类型
      key: '1',
      name: 'John Brown',
  ...
  effects: {
    ...
    * querySupplier({ payload }, { call, put, select }) {
      ...
      const busiIdAll = yield select(state => state.purchaseCheck.newValueShop);
    ...
    * querySupplyList({ payload }, { select, call, put }) {
      ...
      if (data.data && data.data.success) {
        ...
        const distribId = yield select(state => state.purchaseCheck.distribId);
        if (distribId) {
          message.warning(`操作失败，请参考：${data.data.errorInfo}`);
          yield put({
            type: 'querySuccess',
    ...
    * queryOrgList({ payload }, { call, put, select }) { // 请求仓库
      // console.warn("我试试进来了吗")
      yield put({ type: 'showLoading' });
      const newValueStoreList = yield select(state => state.purchaseCheck.newValueStore);
      const data = yield call(queryOrg, parse(payload));
      if (data.data && data.data.success) {
        let newShopList = data.data.data.aclStoreList;
        // console.log("busiIdAll",busiIdAll);
        // console.log("newShopList",newShopList);
        if (!newValueStoreList.length) {
    ...
    * querySupplyListUpdate({ payload }, { put, select }) {
      // console.warn("我试试进来了吗")
      // const storeId = yield select(state => state.purchaseCheck.storeId);
      const { distribId, billListNo, pagination, busiListId, storeListId, listStatus, filterDataRange } = yield select(state => state.purchaseCheck);
      const pageNo = payload.pageNo || pagination.current;
      const pageSize = payload.pageSize || pagination.pageSize;
      // const bussDate = yield select(state => state.purchaseCheck.bussDate);
      yield put({
    ...
    * update({ payload }, { call, put }) {
      ...
      // const path = '/stock/purchaseCheck/supplyOrderAddNew';
    ...
    * closeScmDirect({ payload }, { call, put }) {
      ...
      if (data.data && data.data.success) {
        ...
        const path = '/stock/purchaseCheck';
        yield put(routerRedux.push(path));
        // yield put({
        //   type: 'querySuccess',
        //   payload: {
    ...
    * queryfind({ payload }, { put }) {
      ...
      const path = `/stock/purchaseCheck/supplyOrderFind/${timeId}`;
      yield put(routerRedux.push(path));
    ...
    * addScmDirectManual({ payload }, { call, put, select }) {
      yield put({ type: 'showLoading' });
      const dataSourceNew = yield select(state => state.purchaseCheck.dataSourceNew);
      const newUpdate = yield select(state => state.purchaseCheck.update);
      // yield put({
    ...
    * addNewList({ payload }, { put }) { // 新增采购订单跳转
      ...
      const timeId = moment().valueOf();
      const path = `/stock/purchaseCheck/supplyOrderAddNew/${timeId}`;
    ...
    * editableMem({ payload }, { put, select }) { // 初始化数据
      const data = yield select(state => state.purchaseCheck.dataSource);
    ...
    * updateEditableMem({ payload }, { put, select }) { // 回车跳转新的边框
      const stateMem = yield select(state => state.purchaseCheck.editableMem);
      const dataSource = yield select(state => state.purchaseCheck.dataSource);
    ...
    * addTable({ payload }, { put, select }) { // 新增一行
      let newData = yield select(state => state.purchaseCheck.newData);
      const dataSource = yield select(state => state.purchaseCheck.dataSource);
    ...
    * findTreeList({ payload }, { call, put }) { // 查找物资类别
      // const depotId = yield select(state => state.purchaseCheck.depotId);
      // const storeId = yield select(state => state.purchaseCheck.storeId);
      yield put({ type: 'showLoading' });
    ...
    * exportModalListAll({ payload }, { put, select }) { // 选择全部数据
      const newArrayAll = yield select(state => state.purchaseCheck.dataSource);
      const newValue = yield select(state => state.purchaseCheck.exportModalList);
      const modalRowIndex = yield select(state => state.purchaseCheck.modalRowIndex);
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        ...
        if (location.pathname === '/stock/purchaseCheck') {
        ...
        const reFind = pathToRegexp('/stock/purchaseCheck/supplyOrderFind/:storeIdFindKey');
        ...
        const re = pathToRegexp('/stock/purchaseCheck/supplyOrderAddNew/:storeIdKey');
        ...
        if (match) {
          ...
          if (supplyOrderAll) {
            ...
            dispatch({ // 获取收货机构
              type: 'addDepotId',
              payload: { orgType: 1, rows: 10, storeId: supplyOrderAll.distribId },
            });
          }
        }
        // if (location.pathname === '/stock/purchaseCheck/supplyOrderAddNew') {
